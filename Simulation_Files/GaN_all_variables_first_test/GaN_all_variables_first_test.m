% This was a first test to see the basic behavior of GaN when sweeping over
...
% but it is still a good idea to leave a dummy value
r_sigma = 10;
...
center_radiis = [120, 130, 140, 150, 160];
...
Ndistributions = 10;
% This is the radial distribution. It can be set to be any function which
% returns a random radius with an input of 0 (or no input at all). It is
% defined using lambda notation for convenience.
...
% The final results of this simulation showed us that the behavior of GaN
% is remarkably similar with a normal distribution to how it is with a
% delta distribution. We conjecture that this is largely because of how GaN
% is only a forward-scatterer, so coupling might not change much over small
% radial variation.