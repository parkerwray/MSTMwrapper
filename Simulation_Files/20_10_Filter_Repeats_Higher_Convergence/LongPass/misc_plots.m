...
%sca = 
I = squeeze((mean(Ipar,3)+mean(Iper,3))./2);
reflectances = zeros(length(center_radiis), length(wavelengths));
for i = 1:length(center_radiis)
    for j = 1:length(fill_fractions)
        sca(i, :) = fill_fractions(j) * wopt(i) * ...
...
legend("Coupled data", ..."Isolated data", 
    "Ideal");
set(gca, 'FontSize', 24);
       box on
       pbaspect([1,1,1]);
...
%% Compare multiple sets of reflectances

reflectances  = get_MSTM_reflectances_single_ff(I , qsistats , qsdstats , ...
    center_radiis, wopt, fill_fractions);
reflectances1 = get_MSTM_reflectances_single_ff(I1, qsistats1, qsdstats1, ...
    center_radiis, wopt, fill_fractions);
reflectances2 = get_MSTM_reflectances_single_ff(I2, qsistats2, qsdstats2, ...
    center_radiis, wopt, fill_fractions);
figure,
hold on
plot(wavelengths, reflectances );
plot(wavelengths, reflectances1);
plot(wavelengths, reflectances2);
hold off
legend("Aggregated", "First 25", "Second 25");
