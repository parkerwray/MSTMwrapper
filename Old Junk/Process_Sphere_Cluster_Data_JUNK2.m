...
figure, 
hold on 
plot(wavelengths, (angle(squeeze(((-1).*(-1i)^(1+1)).*modes.Amn(2,:,352:702)))))
plot(mie.lda, angle(mie.an(:,1)),':')
hold off

% figure, 
% hold on 
% plot(wavelengths, abs(squeeze(modes.Amn(2,:,1:351))))
% plot(mie.lda, abs(mie.an(:,1)),':')
% hold off
...
hold on 
plot(wavelengths, 100.*(Rlum), 'b', 'LineWidth', 6)
...
pbaspect([1 1 1])
set(gca,'FontSize',34)
legend('Reflection (calc.)', 'Reflection (FDTD)',...
    'Transmission (calc.)', 'Transmission (FDTD)','FontSize', 34)
