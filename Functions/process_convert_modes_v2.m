...
function [A, B, Ax, Bx, size_param,...
            ...
            Aper, Bper, Axper, Bxper] = ...
                process_convert_modes_v2(Max_Order, sphere_modes_par, sphere_modes_per)
%{ 
...
OUTPUT:
The output of this code is A, B, Ax, Bx. In all cases the dimension are: 
               [lda, pol, n, m]
In the case that a single lda is specified, the dimension is removed such
that the dimensions are:
               [pol, n, m] 
                
You MUST specify the parallel and perpendicluar polarizations. This is
because the Ax and Bx modes deal with polarization conversion. 
...
% Get Perpendicular Modes
    ...
    A = squeeze(permute(A,[2,1,3,4]));
    B = squeeze(permute(B,[2,1,3,4]));
    Ax = squeeze(permute(Ax,[2,1,3,4]));
    Bx = squeeze(permute(Bx,[2,1,3,4]));
